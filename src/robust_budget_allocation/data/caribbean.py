...
def build_canonical(
    ...
    for row in chn_rows:
        ...
        # The HURDAT event category is the storm-lifetime maximum. CHN's value
        # remains the local 60-nm-region maximum; the two meanings are retained.
        hurdat_cat = str(storm["historical_category"])
        chn_cat = str(row["chn_original_category"]).upper()
        row["hurdat_original_category"] = hurdat_cat
        ...
        row["adopted_category"] = hurdat_cat
        row["adoption_rule"] = "HURDAT category adopted when HURDAT and CHN differ"
        confirmed[(str(row["hurdat_storm_id"]), str(row["affected_country_code"]))]["CHN"].append(row)
        if row["category_conflict_flag"] == "YES":
            conflicts.append({
                "conflict_id": f"CATEGORY-{row['chn_evidence_id']}", "conflict_type": "HURDAT_CHN_CATEGORY",
                ...
                "notes": "Original values remain in normalized/evidence tables.",
            })
    ...
    for (storm_id, iso), grouped in sorted(confirmed.items()):
        ...
        adopted_categories = sorted({str(x.get("adopted_category", "")) for x in chn if x.get("adopted_category")})
        impact_category = adopted_categories[0] if len(adopted_categories) == 1 else ""
        if len(adopted_categories) > 1:
            conflicts.append({
                "conflict_id": f"IMPACT-CATEGORY-{storm_id}-{iso}", "conflict_type": "MULTIPLE_CHN_IMPACT_CATEGORIES",
                "hurdat_storm_id": storm_id, "country_code": iso, "source_a": "CHN_LOCATIONS",
                "value_a": "|".join(adopted_categories), "source_b": "", "value_b": "",
                "resolution": "LEAVE_COUNTRY_IMPACT_CATEGORY_BLANK", "status": "UNRESOLVED",
                "notes": "Different CHN sites for one territory reported different date-specific intensity.",
            })
        event_country.append({
            ...
            "country_impact_category": impact_category,
            ...
            "transformation_rule": "included iff at least one confirmed CHN explicit or EM-DAT explicit matched country-impact record",
            "notes": "Not an assertion of the unpublished Balcik affected-country mapping.",
        })
    ...
    for event_id in event_ids:
        ...
        events.append({
            ...
            "season_year": storm["season_year"], "event_date": storm["start_date"].isoformat(),
            "event_end_date": storm["end_date"].isoformat(), "two_week_period": "",
            ...
            "notes": "Source-based reconstruction; not exact recovery of the unpublished Balcik event table. Two-week period unresolved and out of R6-I historical source-layer scope.",
        })
...
def provenance_rows() -> list[dict[str, object]]:
    definitions = [
        ...
        ("historical_category", "NOAA HURDAT2", "maximum sustained wind in knots", "DERIVED", "Saffir-Simpson thresholds"),
        ("severity_class", "Balcik et al. (2019)", "H2 or lower/H3/H4-H5 rule", "DERIVED", "Mild/Strong/Very Strong mapping"),
        ...
        ("country_impact_category", "CHN + NOAA HURDAT2", "local CHN record and HURDAT storm maximum", "RECONSTRUCTED", "Adopt HURDAT category; blank if multiple local evidence values conflict"),
...
def reconstruct() -> dict[str, object]:
    ...
    for row in chn_rows:
        evidence.append({
            ...
            "chn_original_category": row["chn_original_category"], "hurdat_original_category": row.get("hurdat_original_category", ""),
            "conflict_flag": row.get("category_conflict_flag", ""), "adopted_category": row.get("adopted_category", ""),
    ...
    for row in emdat_evidence:
        evidence.append({
            ...
            "chn_original_category": "", "hurdat_original_category": "", "conflict_flag": "", "adopted_category": "",
    ...
    chn_columns = ["chn_evidence_id", "paper_country_code", "affected_country", "affected_country_code", "chn_site_code", "chn_site_name", "event_date", "storm_name", "wind_mph", "chn_original_category", "closest_point_of_approach_miles", "hurdat_storm_id", "match_status", "candidate_hurdat_ids", "hurdat_original_category", "category_conflict_flag", "adopted_category", "adoption_rule", "source_dataset", "source_reference", "source_locator", "source_url", "source_sha256", "proximity_rule", "provenance_class"]
    ...
    event_columns = ["event_id", "source_storm_id", "storm_name", "season_year", "event_date", "event_end_date", "two_week_period", "historical_category", "severity_class", "max_wind", "max_wind_unit", "source_dataset", "source_reference", "source_locator", "provenance_class", "reconstruction_status", "transformation_rule", "notes"]
    event_country_columns = ["event_id", "source_storm_id", "storm_name", "season_year", "event_date", "two_week_period", "historical_category", "severity_class", "country_impact_category", "affected_country", "affected_country_code", "evidence_basis", "evidence_ids", "no_affected", "no_affected_status", "total_affected", "total_affected_status", "event_year_population", "population_unit", "no_affected_pct_population", "total_affected_pct_population", "native_demand_basis", "native_demand_value", "native_demand_unit", "demand_cap", "source_dataset", "source_reference", "source_locator", "provenance_class", "reconstruction_status", "transformation_rule", "notes"]
    ...
    manifest = {
        "schema_version": 1, "dataset_identity": "R6_I_SOURCE_BASED_INDEPENDENT_CARIBBEAN_HISTORICAL_RECONSTRUCTION",
        ...
        "severity_counts": dict(sorted(severity_counts.items())), "match_status_counts": dict(sorted(status_counts.items())),
        ...
        "scientific_scope": {"qfr_parameter_calibration_performed": False, "water_vaccine_crackers_mapping_performed": False, "b_ref_calculated": False, "formal_experiment_matrix_frozen": False, "m0_m1_m2_scientific_runs": 0, "formal_e1_e5_scientific_runs": 0, "family_kit_demand_generated": False, "r6_ii_started": False},
    }
