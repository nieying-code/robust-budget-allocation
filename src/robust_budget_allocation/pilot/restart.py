...
def gates():
    ...
    try:
        ...
        if source_gate() != before or manifest() != payload["source"]:
...
def validate_worker(folder, request):
    ...
    check_seal(claim)
    if request["source"] != claim["source"]:
    ...
    with exclusive_file_lock(ROOT / "outputs/pilot/.locks/n6pilot02.lock", timeout_seconds=0):
        ...
        for index, config in enumerate(reg["configs"]):
            ...
            for method in execution_order(index):
                ...
                try:
                    if manifest() != source or source_gate() != before:
        ...
        try:
            if source_gate() != before or manifest() != source:
...
def replay_restart(bundle):
    ...
    for saved, (run_id, config, method) in zip(bundle["runs"], expected, strict=False):
        ...
        if row["source"] != bundle["source"] or row.get("purpose") != "pilot":
    ...
    if bundle["batch_start"]["source"] != bundle["source"] or bundle["gates"]["source"] != bundle["source"]:
